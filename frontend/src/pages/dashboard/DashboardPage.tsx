    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Personalized jobs and walk-ins — no search required
          </p>
        </div>
      ...
      <DashboardSection title="Browser Automation Status" viewAllHref="/browser-automation">
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card><p className="text-xs text-slate-500">Last Browser Activity</p><p className="text-sm font-medium">{data.statistics.browser_last_activity ?? '—'}</p></Card>
              ...
              <div className="space-y-2">
                {data.recent_cover_letters.map((item) => (
                  <div key={item.id} className="flex items-center justify-between rounded-lg border border-slate-200 p-3 text-sm dark:border-slate-700">
                    ...
                    <Badge variant="info">Quality {item.quality_score ?? '—'}</Badge>
                  </div>
                ))}
              </div>
            ) : (
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <DashboardSection title="Cover Letter Generation Statistics">
          <Card>
            ...
            <p className="mt-3 text-xs text-slate-500">Average quality score: {data.cover_letter_statistics.average_quality_score ?? '—'}</p>
          </Card>
        </DashboardSection>
      </div>
              ...
              <div className="space-y-2">
                {data.recent_tailored_resumes.map((item) => (
                  <div key={item.id} className="flex items-center justify-between rounded-lg border border-slate-200 p-3 text-sm dark:border-slate-700">
                    ...
                    <Badge variant="info">ATS {item.ats_score ?? '—'}</Badge>
                  </div>
                ))}
              </div>
            ) : (
