...
class ApplicationService:
    ...
    def list(
        ...
        serialized = self._serialize_applications(user, items)
    ...
    def _serialize_applications(self, user: User, applications: list[Application]) -> list[ApplicationRead]:
        if not applications:
            return []

        summaries = InterviewService(self.db).summaries_for_jobs(user, [app.job_id for app in applications])
        result: list[ApplicationRead] = []
        for app in applications:
            base = ApplicationRead.model_validate(app)
            result.append(base.model_copy(update=summaries.get(app.job_id, {})))
        return result

    def serialize_application(self, user: User, app: Application) -> ApplicationRead:
        summaries = InterviewService(self.db).summaries_for_jobs(user, [app.job_id])
        base = ApplicationRead.model_validate(app)
        return base.model_copy(update=summaries.get(app.job_id, {}))
